...
import type {LockSeatRequest, Seat} from './types.ts';
...
export const seatsApi = createApi({
    ...
    endpoints: (builder) => ({
        getSeats: builder.query<{ data: Seat[] }, { eventId: number; venueId: number }>({
        ...
        unlockSeat: builder.mutation<void, LockSeatRequest>({
            query: (body) => ({
                url: appConfig.booking.api.path.seatsLocks,
                method: 'DELETE',
                body,
